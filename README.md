...
### Размер данных <br>
Возьмем MAU и умножим на размер данных для каждой таблицы:<br>
- **Users:** <br>
16(UUID) + 50(email) + 50(password) = 116 * 24млн (в месяц) = 2.593 Гб
<br><br>
- **Sessions:** <br>
50(session_id) + 16(UUID) +  8(date) = 74 * 24млн (в месяц) = 1.654 Гб
<br><br>
- **Parking_lots:** <br>
...
<br><br>
- **Favourites:** <br>
4(id) + 16(UUID) +  4(parking_lot_id) = 24 * 24млн (в месяц) * 10 (максимальное количество видов) = 0.536 Гб
...
- Таблица **users**: Для этой таблицы лучше всего использовать реляционную базу данных, такую как PostgreSQL или MySQL. Они обеспечивают эффективное хранение и операции с данными пользователей, такие как аутентификация и управление доступом;
- Таблица **parking_lots, rows**: Для информации о парковках лучше всего использовать географическую базу данных, такую как PostGIS (расширение PostgreSQL) или MongoDB с поддержкой геоиндексов. Это позволит эффективно хранить и выполнять пространственные запросы к парковкам, например, поиск ближайших парковок к определенной точке на карте;
- Таблица **favourites, views**: Для хранения информации о избранных парковках пользователя также можно использовать реляционную базу данных, поддерживающую быстрый доступ к данным, такой как PostgreSQL или MySQL. Для повышения производительности можно также использовать кэширующие решения (например Redis) для хранения популярных избранных парковок или информации о последних избранных парковках пользователя.

### Индексы
 - Таблица **users**: Индекс по email, т.к. он используется для аутентификации и поиска пользователей по их электронной почте;
 - Таблица **sessions**: Индекс по id пользователя, чтобы быстро находить все сессии конкретного пользователя;
 - Таблица **parking_lots**: Индекс по координатам, если часто выполняются запросы на поиск ближайших парковок к определенной точке на карте;
 - Таблица **views**: Индекс по id камеры, если часто выполняются запросы на поиск видов камер, принадлежащих конкретной парковке;
 - Таблица **favourites**: Индекс по id парковки, если часто выполняются запросы на поиск всех пользователей, отметивших определенную парковку как избранную.

### Денормализация
В таблице **views** можно добавить поле, содержащее адрес парковки, чтобы при запросах избежать объединения с таблицей parking_lots.<br>
В таблице **rows** можно добавить поля, содержащие адрес парковки и количество свободных мест на парковке, для хранения информации о парковке, чтобы при запросах избежать объединения с таблицей parking_lots.

### Шардинг
- users, favourites по user_id;
- parking_lots, views, rows по parking_lot_id.
...
3. С помощью этих данных создается матрица счетчиков по размеру кадра.
4. Если область машины попала на нужный счётчик, то он инкрементируется. Чем больше величина счетчика, тем более вероятно, что это парковочное место.
