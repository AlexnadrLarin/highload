...
- Общий объем, который занимает карта равняется 200 Тб

### Общий объем памяти за месяц равен:
200 * 1024 + (140 + 74 + 116 + 240) * 24 * 10^6 / 1024 / 1024 / 1024 / 8 = 204801.593 Гб
...
- **Users:** <br>
16(UUID) + 50(email) + 50(password) = 116 * 24млн * 1.5 = 0.4862 Гб
<br><br>
- **Sessions:** <br>
...
### Индексы
 ...
 - Таблица **parking_lots**: Индекс по ;
...
| RabbitMQ | Брокер сообщений | Возможность снизить нагрузку на БД |
| Kubernetes | DevOps | Предоставляется автоматическая масштабируемость, отказоустойчивость и удобное управление ресурсами |

## 9. Схема проекта
![image](https://github.com/AlexnadrLarin/highload/assets/47739783/c02e3405-1f65-4b3a-bb52-cc83868b9a12)

| Сервис | Область применения |
|---------------------|---------------------|
| Auth | Авторизация, регистрация пользователя, управление сессиями |
| HeadService | Выдача пользователю информации о парковках, управление избранным |
| Analyzer | Распознавание мест |
